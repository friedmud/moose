//* This file is part of the MOOSE framework
//* https://www.mooseframework.org
//*
//* All rights reserved, see COPYRIGHT for full restrictions
//* https://github.com/idaholab/moose/blob/master/COPYRIGHT
//*
//* Licensed under LGPL 2.1, please see LICENSE for details
//* https://www.gnu.org/licenses/lgpl-2.1.html

#include "gtest/gtest.h"

#include "CircularBuffer.h"

#include <algorithm> // std::set_symmetric_difference

TEST(CircularBuffer, test)
{
  MooseUtils::CircularBuffer<int> buffer(4);

  EXPECT_EQ(buffer.size(), 0);
  EXPECT_EQ(buffer.capacity(), 4);

  buffer.push_back(1);
  buffer.push_back(2);
  buffer.push_back(3);
  buffer.push_back(4);

  EXPECT_EQ(buffer.beginPos(), 0);
  EXPECT_EQ(buffer.endPos(), 4);

  auto & data = buffer.data();

  EXPECT_EQ(data[0], 1);
  EXPECT_EQ(data[1], 2);
  EXPECT_EQ(data[2], 3);
  EXPECT_EQ(data[3], 4);

  buffer.erase(2);

  EXPECT_EQ(*buffer.begin(), 3);

  EXPECT_EQ(buffer.beginPos(), 2);
  EXPECT_EQ(buffer.endPos(), 4);

  buffer.erase(2);

  EXPECT_EQ(buffer.beginPos(), 0);
  EXPECT_EQ(buffer.endPos(), 0);

  buffer.push_back(1);
  buffer.push_back(2);
  buffer.push_back(3);
  buffer.push_back(4);

  EXPECT_EQ(buffer.beginPos(), 0);
  EXPECT_EQ(buffer.endPos(), 4);

  buffer.push_back(5);

  EXPECT_EQ(buffer.beginPos(), 0);
  EXPECT_EQ(buffer.endPos(), 5);

  EXPECT_EQ(buffer.size(), 5);
  EXPECT_EQ(buffer.capacity(), 10);

  buffer.erase(buffer.begin() + 2);

  EXPECT_EQ(buffer.beginPos(), 2);
  EXPECT_EQ(buffer.endPos(), 5);

  for (unsigned int i = 6; i < 11; i++)
    buffer.push_back(i);

  EXPECT_EQ(buffer.beginPos(), 2);
  EXPECT_EQ(buffer.endPos(), 10);

  buffer.push_back(11);

  EXPECT_EQ(buffer.beginPos(), 0);
  EXPECT_EQ(buffer.endPos(), 9);
  EXPECT_EQ(buffer.size(), 9);
  EXPECT_EQ(buffer.capacity(), 10);

  for (unsigned int i = 0; i < 9; i++)
    EXPECT_EQ(buffer[i], 3 + i);

  {
    unsigned int i = 0;

    for (const auto & val : buffer)
    {
      EXPECT_EQ(val, 3 + i);
      i++;
    }
  }

  buffer.erase(buffer.begin() + 3);

  EXPECT_EQ(buffer.beginPos(), 3);
  EXPECT_EQ(buffer.endPos(), 9);
  EXPECT_EQ(buffer.size(), 6);
  EXPECT_EQ(buffer.capacity(), 10);

  EXPECT_EQ(buffer[2], 8);

  std::vector<int> dummy = {1, 2, 3, 4};

  buffer.swap(dummy);

  EXPECT_EQ(buffer.beginPos(), 0);
  EXPECT_EQ(buffer.endPos(), 4);
  EXPECT_EQ(buffer.size(), 4);
  EXPECT_EQ(buffer.capacity(), 4);

  buffer.erase(2);
  buffer.push_back(5);

  EXPECT_EQ(buffer.beginPos(), 0);
  EXPECT_EQ(buffer.endPos(), 3);
  EXPECT_EQ(buffer.size(), 3);
  EXPECT_EQ(buffer.capacity(), 4);

  buffer.erase(2);
  buffer.append({6, 7, 8, 9, 10});

  EXPECT_EQ(buffer.beginPos(), 0);
  EXPECT_EQ(buffer.endPos(), 6);
  EXPECT_EQ(buffer.size(), 6);
  EXPECT_EQ(buffer.capacity(), 12);

  EXPECT_EQ(buffer.empty(), false);

  buffer.clear();

  EXPECT_EQ(buffer.empty(), true);
}
