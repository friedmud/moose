...
template <>
InputParameters
validParams<HierarchicalGridPartitioner>()
{
  InputParameters params = validParams<MoosePartitioner>();

  params.addRequiredParam<unsigned int>("nx_nodes", "Number of compute nodes in the X direction");
  params.addParam<unsigned int>("ny_nodes", 0, "Number of compute nodes in the Y direction");
  params.addParam<unsigned int>("nz_nodes", 0, "Number of compute nodes in the Z direction");

  params.addRequiredParam<unsigned int>("nx_procs",
                                        "Number of processors in the X direction within each node");
  params.addParam<unsigned int>(
      "ny_procs", 0, "Number of processors in the Y direction within each node");
  params.addParam<unsigned int>(
